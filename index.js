...
const BarBarKey = 'Q7MLdUnVSkXgLmQE9liX'
...
const client = new WAConnection()
   ...
   console.log(color('♥','white'),color('♥','red'),color('♥','white'),color('MADED','white'),color('BY','red'),color('SANG','white'),color('LORD BOTZ','yellow'))
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\Selamat datang di group *${mdata.subject}* yang betah ya di sini by HARUKA`
				...
				teks = `Yah Anak anjing keluar hati hati yak moga ketabrak truk amin @${num.split('@')[0]}👋🍁`
...
// ANTI LINK GRUP
		        ...
		        if (isGroupAdmins) return reply('Admin Grup Mah Bebas Yah Kan:D')
		        ...
		        setTimeout( () => {
			        ...
			        client.groupRemove(from, [kic]).catch((e)=>{reply(`*HARUKA HARUS JADI ADMINâ—*`)})
			...
			if (args.length < 1) return reply(ind.wrongf())
				 ...
				 if (isLimit(sender)) return reply(ind.limitend(pusname))
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson('https://tobz-api.herokuapp.com/api/cry?apikey=alpinbotwa', {method: 'get'})
...
case 'sound9':
tujuh = fs.readFileSync('./assets/sound9.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound10':
tujuh = fs.readFileSync('./assets/sound10.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break 
case 'sound11':
tujuh = fs.readFileSync('./assets/sound11.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break 
case 'sound12':
tujuh = fs.readFileSync('./assets/sound12.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break 
case 'sound13':
tujuh = fs.readFileSync('./assets/sound13.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break
case 'sound14':
tujuh = fs.readFileSync('./assets/sound14.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break  
case 'sound15':
tujuh = fs.readFileSync('./assets/sound15.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break 
case 'sound16':
tujuh = fs.readFileSync('./assets/sound16.mp3');
client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
break   
case 'hartatahta':
...
reply('「LORD」MADED BY SANGLORD:v')
...
break
		...
		break
			...
			case 'hentai':
				   ...
				   if (!isGroup) return reply(ind.groupo())
                                      if (!isGroupAdmins) return reply(ind.admin())
				    if (isLimit(sender)) return reply(limitend(pushname2))
						if (!isNsfw) return reply(' *belum di aktifin tod!* ')
		...
		case 'resepmasakan':  
			...
			if (!isGroupAdmins) return reply(ind.admin())
					...
					teks = `\`\`\`Ini adalah list group ՏᎪᏦႮᎡᎪ ᏴϴͲᏃ :\n\n\`\`\``
	...
	case 'hobby':
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname)) 
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Itu pacar aku eh buka dia owner aku bro',MessageType.text, { quoted: mek} )
				...
				case 'promote':
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							...
							teks += `asek dapat makanan,otw mengkickmu, 🏃 bran kang gay darwin ajg :\n`
		...
		case 'next':
			...
			if (body.startsWith(`${prefix}${command}`)) {
                  ...
                  reply(`*${pushname}*, Command *${prefix}${command}* Tidak Ada Di Dalam *${prefix}menu Bot sakura!*`)

		const none = fs.readFileSync('./assets/sakura.mp3');
